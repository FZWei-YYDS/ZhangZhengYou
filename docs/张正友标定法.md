...
Ax = \lambda x (x \ne 0)
$$
则称 $\lambda$ 是矩阵 $A$ 的一个**[特征值](https://www.sohu.com/a/72049830_103414)**，$x$ 是矩阵 $A$ 的对应于特征值 $\lambda$ 的**特征向量**。
...
因为取逆的运算太耗费时间，一般我们会将 $W$ 的这 $n$ 个特征向量标准化，即满足：${\left\| {{w_i}} \right\|_2} = 1$，$w_i^Tw=1$。此时 $W$ 的 $n$ 个特征向量为标准正交基，满足：$W^TW=I$，即$W^T=W^{-1}$，也就是 $W $为酉矩阵（转置的运算远远小于求逆）。
$$
A = W S W^T
$$
